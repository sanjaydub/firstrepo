...
public class SampleSuiteITest {
    ...
    //        httpRequest.request(io.restassured.http.Method.GET)
        ...
        @Test
        public void testRectangleAreaCalc(ITestContext tCon) {
            Assert.assertEquals(new Rectangle(5,5).calculateArea(),25, tCon.getCurrentXmlTest().getName() + " : is failed");

        }

        @Test
        public void testRectanglePeriCalc(ITestContext tCon) {
            Assert.assertEquals(new Rectangle(5,5).calculatePerimeter(),20, tCon.getCurrentXmlTest().getName() + " : is failed");
